...
import { RepairCoordinator, RepairRequestError } from "./repair.js";
...
export interface ClinicServerOptions {
  repairCoordinator?: RepairCoordinator;
}

export async function startClinicServer(report: SkillDoctorReport | ReportStore, port: number, options: ClinicServerOptions = {}): Promise<ClinicServer> {
  const store = report instanceof ReportStore ? report : new ReportStore(report);
  const repairCoordinator = options.repairCoordinator ?? new RepairCoordinator({ store, scanFn: async () => store.getReport() });
  ...
  const server = createServer(async (request, response) => {
    ...
    if (url.pathname === "/api/repairs") {
      ...
      try {
        ...
        sendJson(response, 202, repairCoordinator.startPatientRepair(body.patient_id));
      } catch (error) {
        sendJson(response, error instanceof RepairRequestError ? error.statusCode : 400, {
    ...
    if (url.pathname === "/api/repairs/all") {
      if (request.method !== "POST") {
        sendJson(response, 405, { message: "一键治疗接口只支持 POST 请求" });
        return;
      }

      try {
        sendJson(response, 202, repairCoordinator.startAllRepairs());
      } catch (error) {
        sendJson(response, error instanceof RepairRequestError ? error.statusCode : 400, {
          message: error instanceof Error ? error.message : "治疗请求无法解析"
        });
